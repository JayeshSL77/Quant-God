        ...
        def wrapper(*args, **kwargs):
            ...
            for i in range(retries + 1):
                try:
                    ...
                    last_exception = e
                    # Check for rate limit errors (429) or overloaded (503)
                    error_str = str(e).lower()
                    if "429" in error_str or "rate limit" in error_str or "quota" in error_str or "429" in str(getattr(e, 'status_code', '')):
                         if i < retries:
                             sleep_time = delay + random.uniform(0, 0.5)
                             time.sleep(sleep_time)
                             delay *= backoff_factor
                             continue
                    raise e # Re-raise if not a rate limit issue or out of retries
...
# Visual RAG imports
try:
    from api.core.charting.generator import detect_chart_intent, generate_relevant_chart, ChartGenerator
...
5. THESIS ARTICULATION
   - State future outlook: Positive Expected / Neutral / Pessimistic (NOT Overweight/Underweight)
...
- Use markdown headers (# for main sections like # THESIS, # RISKS)
- Top-level points use dashes (-) for bullet points
- Sub-points under a bullet MUST be nested with indentation (2 spaces + dash)
- Keep structure flat when possible - avoid deep nesting
- NO emojis, NO casual language
- Use precise financial terminology

SENTIMENT HIGHLIGHTING (CRITICAL):
- Use [+] prefix for POSITIVE insights and [-] prefix for NEGATIVE/RISK insights
- LIMIT: Only 1-2 highlighted lines per section MAXIMUM
- Choose only the MOST significant insight in each section to highlight
- Most lines should have NO sentiment prefix (neutral/unmarked)
- Example: In a section with 4 bullets, only 1 should have [+] or [-]
- NEVER append meta-labels such as "Word Count", "Style", or "Analysis summary"
- DO NOT summarize your own instructions
- Finish your response with the THESIS section
- At the very end, generate a section "## RELATED QUESTIONS" with exactly 3 highly relevant follow-up questions
- Format related questions as a bulleted list: "- Question?"
...
EXAMPLE OUTPUT FORMAT (FOLLOW THIS EXACTLY - NOTE LIMITED HIGHLIGHTING):

# VALUATION CONTEXT
- Current P/E of 22x sits 15% below its 5-year average of 26x
- [+] Trading at discount to sector peers (18x) with superior ROE of 18%
- 52-week range position suggests entry point

# RISKS
- Demand volatility in key markets
- [-] Attrition rising to 13% from historical lows, wage inflation pressures
- Competitive intensity from hyperscalers

# THESIS
- Outlook: Positive Expected (Upside: 15-20%)
- Fair Value Range: Rs 1,800-2,000 based on 25x forward P/E
- [+] Key Catalysts:
  - Q4 order inflows expected in March
  - New product launch in H1 FY26
...
def build_dynamic_prompt(
    ...
    analysis_mode: str = "deep_research"
) -> str:
    """
    Build a dynamic prompt that ONLY includes sections where we have actual data.
    Adjusts sections based on analysis_mode:
      - summary: Only THESIS section
      - business: All sections except VALUATION CONTEXT
      - deep_research: Full comprehensive analysis
    """
    
    sections = []
    
    # VISUAL CONTEXT - Implicitly handled via charts, NO explicit text section needed.
    # The '0. VISUAL INSIGHTS' section is REMOVED to solve indexing issues.

    
    # VALUATION - only for deep_research mode
...
""")
    ...
    # EARNINGS QUALITY - only if we have actual margin/growth data (business & deep_research)
    if (has_margin_data or has_growth_data) and analysis_mode != "summary":
        sections.append("""
2. EARNINGS QUALITY
...
""")
    ...
    # MANAGEMENT SIGNALS - only if we have actual transcripts/quotes (business & deep_research)
    if has_earnings_transcript and analysis_mode != "summary":
        sections.append("""
3. MANAGEMENT SIGNALS
...
""")
    ...
    # RISKS - always include for business & deep_research
...
""")
    ...
    # THESIS - always include (all modes)
    sections.append("""
5. THESIS
...
- Each section header must use markdown: # followed by number and name (e.g. # 1. VALUATION CONTEXT)
- ALL bullet points must use simple dashes (-) only
- NEVER use asterisks (*), dots (•), arrows (→), or any other bullet style
- Sub-bullets should be indented with two spaces and also use dashes
- Keep formatting consistent throughout the entire response
- NEVER append meta-labels such as "Word Count", "Style", or "Analysis summary"
- DO NOT summarize your own instructions
- Finish your response with the THESIS section
...
1. LANGUAGE MATCHING (STRICT): 
   ...
   - If User asks in ENGLISH -> output MUST be 100% ENGLISH. No Hinglish.
   - If User asks in HINDI/HINGLISH -> output MUST be HINGLISH.
   - CRITICAL: Do NOT mix languages unless explicitly requested.

2. FORMATTING BOUNDARIES (STRICT):
   - The '5. THESIS' section is the FINAL section.
   - Do NOT write any concluding summary, closing remarks, or conversational text after the Thesis.
   - STOP generating immediately after the Thesis catalysts.

2. SMART EDUCATION:
   - If the user asks basic questions, explain complex terms (like P/B, EBITDA, ROCE) in parentheses.
   - Example: "...high ROE (Return on Equity, matlab company kitna profit bana rahi hai investors ke paise par)..."

CRITICAL: Use ONLY verified numbers from the data below. If a calculation or number isn't provided, don't mention it.

3. SENTIMENT COLORING (STRICT LIMITS - USER RULE):
   - You may use [sentiment] tags to highlight KEY trends, but you must be VERY selective.
   - RULE: MAXIMUM 1 POSITIVE [+] and 1 NEGATIVE [-] highlight per section.
   - CAUTION: Do NOT color every line. If there is no strong signal, do NOT use any tags.
   - Use [+] prefix for STRONG POSITIVE indicators (Growth > 10%, Margin expansion).
   - Use [-] prefix for STRONG NEGATIVE indicators (Decline, Miss).
   - Example: "[+] Revenue up 15%. [-] Margins contracted 200bps." matches the max limit.
...
def build_historical_context(symbol: str, snapshot: Dict[str, Any], long_term: Dict[str, Any]) -> str:
    """
    Build always-on historical context for responses.
    P1: This context is ALWAYS injected into prompts.
    """
    sections = []
    ...
    if high_52w and low_52w and price:
        try:
            ...
            if high_f != low_f:
                ...
                if position < 25:
                    sections.append(f"[+] Currently trading near 52-week lows ({position:.0f}% of range) - potential value entry")
                elif position > 75:
                    sections.append(f"52-week range: Currently at {position:.0f}% of range (₹{low_f:,.0f} - ₹{high_f:,.0f})")
                else:
                    sections.append(f"52-week range: ₹{low_f:,.0f} - ₹{high_f:,.0f} (currently at {position:.0f}% of range)")
        except (ValueError, TypeError):
            pass
    
    # 10-year CAGR
    cagr = long_term.get('cagr_10yr')
    if cagr and cagr != 'N/A':
        try:
            cagr_f = float(cagr)
            if cagr_f > 15:
                sections.append(f"[+] Strong long-term performer: 10-Year CAGR of {cagr_f:.1f}%")
            elif cagr_f > 8:
                sections.append(f"Decent long-term returns: 10-Year CAGR of {cagr_f:.1f}%")
            else:
                sections.append(f"[-] Modest long-term returns: 10-Year CAGR of {cagr_f:.1f}%")
        except (ValueError, TypeError):
            pass
    
    # Historical PE context
    pe = snapshot.get('pe_ratio')
    sector_pe = snapshot.get('sector_pe')
    if pe and sector_pe:
        try:
            pe_f = float(pe)
            sector_pe_f = float(sector_pe)
            if sector_pe_f > 0:
                premium = ((pe_f / sector_pe_f) - 1) * 100
                if premium < -15:
                    sections.append(f"[+] Trading at {abs(premium):.0f}% discount to sector PE")
                elif premium > 30:
                    sections.append(f"[-] Trading at {premium:.0f}% premium to sector PE")
        except (ValueError, TypeError):
            pass
    
    return "\n".join(sections) if sections else "Historical context: Limited data available"
...
class OrchestratorV2(BaseAgent):
    ...
    def __init__(self):
        ...
        if self.provider == "openai":
            ...
            if api_key:
                ...
                self.gemini_client = genai.GenerativeModel(os.getenv("LLM_MODEL", "gemini-2.0-flash-exp"))
    ...
    def _process_comparison(self, query: str, tickers: List[str], context: Dict[str, Any]):
        ...
        if selected_sources:
            agents = [agent_map[s] for s in selected_sources if s in agent_map]
        else:
        ...
        from api.core.data_sources import get_fundamentals_batch
        ...
        # 4. Generate comparison chart (Disabled per user feedback: "one image is enough")
        chart_data = None
...
"""
    ...
    def process(self, query: str, context: Dict[str, Any]):
        ...
        # Run agents sequentially to avoid rate limits (Gemini Free Tier)
        ...
        # Build market data string dynamically to ONLY include fields with valid data
        historical_context = build_historical_context(symbol, latest, long_term)
        market_data_lines = ["VALUATION METRICS (Current):"]
        def _add_line(label, value, suffix=""):
            if value and value != "N/A":
                market_data_lines.append(f"- {label}: {value}{suffix}")

        _add_line("Price", latest_for_prompt.get('price'), " Rs")
        _add_line("Market Cap", market_data.get('market_cap_formatted'))
        _add_line("PE Ratio", latest_for_prompt.get('pe_ratio'), "x")
        _add_line("Sector PE", latest_for_prompt.get('sector_pe'), "x")
        _add_line("PB Ratio", latest_for_prompt.get('pb_ratio'), "x")
        _add_line("Book Value", latest_for_prompt.get('book_value'), " Rs")

        market_data_lines.append("\nTRADING METRICS:")
        _add_line("52W High", latest_for_prompt.get('high_52w'), " Rs")
        _add_line("52W Low", latest_for_prompt.get('low_52w'), " Rs")
        _add_line("YTD Change", latest_for_prompt.get('ytd_change'), "%")
        _add_line("Beta", latest_for_prompt.get('beta'))

        market_data_lines.append("\nFUNDAMENTALS (TTM):")
        _add_line("EPS", latest_for_prompt.get('eps_ttm'), " Rs")
        _add_line("Dividend per Share", latest_for_prompt.get('dividend_per_share'), " Rs")
        _add_line("Dividend Yield", latest_for_prompt.get('dividend_yield'), "%")

        market_data_lines.append("\nGROW & PROFIT:")
        _add_line("Revenue Growth", latest_for_prompt.get('revenue_growth'), "%")
        _add_line("Profit Growth", latest_for_prompt.get('profit_growth'), "%")
        _add_line("Net Margin", latest_for_prompt.get('net_margin'), "%")
        _add_line("ROE", latest_for_prompt.get('roe'), "%")

        market_data_lines.append("\n10-YEAR PERFORMANCE (VERIFIED):")
        if cagr_10yr != "N/A":
            market_data_lines.append(f"- CAGR: {cagr_10yr}%")
        if long_term.get('change_pct_total') and long_term.get('change_pct_total') != "N/A":
            market_data_lines.append(f"- Total Return: {long_term.get('change_pct_total')}%")

        market_data_lines.append("\nHISTORICAL CONTEXT (ALWAYS INCLUDE IN RESPONSE):")
        market_data_lines.append(historical_context)
        ...
        for chunk in self._stream_institutional(
            ...
            user_context=context.get("user_context"),  # PERSONALIZATION: Inject user persona
            analysis_mode=context.get("analysis_mode", "deep_research")  # Analysis depth mode
        ):
    ...
    def _stream_institutional(
        ...
        Supports analysis_mode for varying response depth.
        """
...
{f"USER PERSONA (Personalize response based on these learned preferences):\n{user_context}" if user_context else ""}
...
"""
        ...
        try:
            if self.provider == "mistral" and self.mistral_client:
                ...
                def generate_stream_mistral():
                     return self.mistral_client.chat.stream(
                        model=os.getenv("LLM_MODEL", "mistral-large-latest"),
                ...
                try:
                    stream_response = generate_stream_mistral()
                except Exception as e:
                    # Fallback to smaller model if large model fails (likely 429)
                    self._log_activity(f"[V2] mistral-large failed ({str(e)}), falling back to mistral-small")
                    
                    @retry_with_backoff(retries=5, initial_delay=4)
                    def generate_stream_fallback():
                         return self.mistral_client.chat.stream(
                            model="mistral-small-latest",
                            messages=[{"role": "user", "content": prompt_content}],
                            temperature=0.3
                        )
                    stream_response = generate_stream_fallback()
                ...
                # Gemini streaming with manual retry for generator
                prompt_content = prompt
                
                @retry_with_backoff(retries=5, initial_delay=4)
                def generate_stream():
                    return self.gemini_client.generate_content(prompt_content, stream=True)
